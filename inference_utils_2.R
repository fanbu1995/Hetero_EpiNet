# 03/09/2021
# modified utility functions for Hetero EpiNet with external infec sources
...
solve_MLE <- function(summaries, X, maxIter=10, tol=1e-4, initEta = 1, 
  ...
  xi = nI_ext/sum(epi_table$infec_time)
  ...
  for(it in 1:maxIter){
    # 3.1 estimate b_E
    ext_Expo = xi * epi_table$infec_time
    ext_poi = glm(Z_infec~X-1+offset(log(ext_Expo)), family = poisson())
    b_E = ext_poi$coefficients %>% as.numeric()
    
    # 3.2 estimate xi
    xi_old = xi
    indiv_effects = exp(c(X %*% b_E))
    xi = nI_ext/sum(indiv_effects * epi_table$infec_time)
